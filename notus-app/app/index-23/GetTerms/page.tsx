...
const comparisonData: { category: string; rows: Row[] }[] = [
  { category: "Pricing", rows: [
    ...
    { feature: "Cheapest paid plan", pc: "$9/mo (Starter)", comp: "$9/mo per website (Starter)" },
    { feature: "Pricing model", pc: "Per plan (multi-domain)", comp: "Per website" },
    { feature: "Unlimited pageviews", pc: "Enterprise ($79/mo)", comp: "Business ($12/mo per site)" },
    { feature: "Unlimited cookie scans", pc: "Pro & Enterprise", comp: "Business plan" },
  ]},
  { category: "Cookie Scanning", rows: [
    { feature: "Automated cookie scans", pc: true, comp: "Business plan" },
    { feature: "Cookie auto-blocking", pc: true, comp: true },
    { feature: "Scheduled scans", pc: true, comp: true },
  ]},
  { category: "Consent Banner", rows: [
    { feature: "Customizable cookie banner", pc: "Advanced (all paid)", comp: "Business plan" },
    { feature: "Remove branding", pc: "Pro & Enterprise", comp: "Business plan" },
    { feature: "Multi-language support", pc: "All languages (Pro+)", comp: "Business plan" },
    { feature: "Auto-language detection", pc: true, comp: "Business plan" },
    { feature: "Geo-targeting", pc: true, comp: "Both plans" },
  ]},
  { category: "Compliance", rows: [
    ...
    { feature: "Regulation monitoring", pc: true, comp: "Business plan" },
  ]},
  { category: "Legal Documents", rows: [
    { feature: "Privacy policy", pc: true, comp: true },
    { feature: "Cookie policy", pc: true, comp: "Starter plan" },
    { feature: "Unlimited policy edits", pc: true, comp: "Business plan" },
    { feature: "HTML policy embeds", pc: true, comp: true },
  ]},
  { category: "Integrations & Support", rows: [
    ...
    { feature: "Multi-user logins", pc: "Up to unlimited", comp: "Partner plan" },
    { feature: "Priority support", pc: "Pro & Enterprise", comp: "Partner plan" },
  ]},
];

const advantages = [
  { icon: DollarSign, title: "Free Plan Available", desc: "PrivacyCrumb offers a free plan. GetTerms has no free tier — starts at $9/mo per site." },
  { icon: Globe, title: "Multi-Domain Plans", desc: "One plan covers multiple domains. GetTerms charges per website, making multi-site costly." },
  { icon: Zap, title: "Full API & Webhooks", desc: "REST API and webhooks for developers. GetTerms doesn't publicly document API access." },
  { icon: Shield, title: "More Features on Base Plan", desc: "Advanced banner, multi-language, and scans on Pro. GetTerms reserves many features for Business." },
];

const Cell = ({ value }: { value: string | true }) => {
  if (value === true) return <CheckCircle size={16} className="text-emerald-500 mx-auto" />;
  if (value === "No free plan" || value === "Not specified") return <span className="text-red-400 text-xs">{value}</span>;
  return <span className="text-xs text-neutral-600 dark:text-neutral-400">{value}</span>;
};

export default function GetTermsComparePage() {
  return (
      ...
      <main className="bg-white dark:bg-neutral-950 min-h-screen">
        <section className="text-center pt-24 pb-16 px-6 relative overflow-hidden grid-bg">
          ...
          <div className="relative z-10 max-w-3xl mx-auto">
            ...
            <FadeIn delay={0.1}><h1 className="text-[2.5rem] md:text-[3.25rem] font-semibold leading-[1.1] mb-5 tracking-tight text-neutral-900 dark:text-white">PrivacyCrumb vs <span className="bg-gradient-to-b from-[#F2A07A] to-coral bg-clip-text text-transparent">GetTerms</span></h1></FadeIn>
            <FadeIn delay={0.2}><p className="text-neutral-500 dark:text-neutral-400 text-base max-w-lg mx-auto leading-relaxed">See how PrivacyCrumb compares to GetTerms across pricing, legal documents, and developer tools.</p></FadeIn>
